...
const TabNavigation = ({ tabs, activeTab, onTabChange }) => {
  ...
  // Get sub-tabs for a category
  const getCategoryTabs = (categoryId) => {
    return tabs.filter(t => t.category === categoryId)
  }

  // Check if a category has tabs (for non-super-admin who may have fewer tabs)
  ...
  const handleCategoryClick = (cat) => {
    if (cat.isCategory) {
      if (openCategory === cat.id) {
        ...
        // If no sub-tab from this category is active, navigate to the default
  ...
  // Check if a primary nav item is active
  const isNavActive = (cat) => {
    if (cat.isCategory) return activeCategory === cat.id
    return activeTab === cat.id
  }

  // Get the label to show for a category when one of its sub-tabs is active
