...
const PaymentSettings = ({ onRefresh }) => {
  ...
  useEffect(() => {
    if (existingConfig) {
      ...
      // Note: We don't load encrypted keys - user must re-enter if changing
      // Note: Convenience fee is now configured in Booking Settings
    }
  }, [existingConfig]);

  // Check if API keys are configured (from existing config)
  const hasApiKeysConfigured = existingConfig?.has_public_key && existingConfig?.has_secret_key;
