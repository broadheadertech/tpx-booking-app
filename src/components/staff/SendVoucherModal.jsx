...
const SendVoucherModal = ({ isOpen, onClose, voucher }) => {
  ...
  const assignVoucherMutation = useMutation(
    api.services.vouchers.assignVoucherByCode
  );
  const sendVoucherEmailAction = useAction(
    api.services.auth.sendVoucherEmailWithQR
  );
  ...
  useEffect(() => {
    if (clients && Array.isArray(clients)) {
      ...
      const assignedUserIds = assignedUsers
        ? assignedUsers.map((user) => user._id || user.id)
        : [];
    ...
    try {
      for (const selectedUser of selectedUsers) {
        try {
          ...
          if (!userId)
            throw new Error(`Invalid user ID for ${selectedUser.username}`);
          if (!staffId)
          ...
          if (emailResult.success) {
            ...
            setSentCount((prev) => {
              ...
              console.log(
                `Incrementing sent count from ${prev} to ${newCount}`
              );
              return newCount;
            });
          } else {
      ...
      const details = selectedUsers.map(
