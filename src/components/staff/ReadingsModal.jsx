...
import { FileText, Printer, RefreshCw, AlertTriangle, Lock } from 'lucide-react'
...
const ReadingsModal = ({ isOpen, onClose, branchInfo, staffInfo }) => {
  ...
  const [mode, setMode] = useState('menu') // 'menu' | 'x' | 'zdone'
  ...
  const close = () => { setMode('menu'); setZResult(null); onClose() }
  ...
  const renderReading = (r, title) => (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <button onClick={() => { setMode('menu'); setZResult(null) }} className="py-3 border border-[#555] text-gray-300 font-semibold rounded-xl hover:bg-[#2A2A2A]">
          Back
        </button>
      </div>
    </div>
...
function readingMeta(r) {
  ...
  return {
    isZ,
    title1: isZ ? 'End Of Day Report' : 'Interim Sales Report',
    title2: isZ ? '(Z-Read)' : '(X-Read)',
...
function generateReadingHTML(r, branchInfo, staffInfo) {
  ...
  return `<!DOCTYPE html><html><head><meta charset="utf-8"><title>${m.title2}</title><style>
    ...
    * { margin:0; padding:0; box-sizing:border-box; }
    ...
    <table>
      ${kv('Reset Counter No.', 'Non-Resettable')}
      ${kv('Z-Counter', m.zCounter)}
