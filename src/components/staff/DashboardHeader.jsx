    ...
    <div className="relative bg-[#050505] border-b border-[#1A1A1A]/30 overflow-hidden" style={{ paddingTop: 'env(safe-area-inset-top)' }}>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-4 md:px-6 lg:px-8">
        <div className="flex items-center justify-between py-2 sm:py-2.5 md:py-3 gap-1.5 sm:gap-2 md:gap-3">
          ...
          <div className="flex items-center space-x-1.5 sm:space-x-2 md:space-x-3 min-w-0 flex-1">
            <div className="w-8 h-8 sm:w-9 sm:h-9 md:w-10 md:h-10 lg:w-11 lg:h-11 bg-[#0A0A0A] rounded-lg sm:rounded-xl flex items-center justify-center shadow-lg ring-1 ring-[#FF8C42]/20 p-1 sm:p-1.5 border border-[#1A1A1A]/50 flex-shrink-0">
              <img
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center space-x-1 sm:space-x-1.5 md:space-x-2 flex-wrap">
                <h1 className="text-[11px] sm:text-sm md:text-base lg:text-lg font-bold text-white tracking-tight truncate">
                  <span className="hidden md:inline">TipunoX Angeles Barbershop</span>
                  <span className="hidden sm:inline md:hidden">TipunoX Angeles</span>
                  <span className="sm:hidden">TipunoX</span>
                </h1>
                <div className="bg-[#FF8C42]/15 backdrop-blur-sm rounded-md px-1 sm:px-1.5 py-0.5 border border-[#FF8C42]/25 flex-shrink-0 hidden sm:block">
                  <span className="text-[9px] sm:text-[10px] font-semibold text-[#FF8C42]">v{APP_VERSION}</span>
                </div>
              </div>
              <div className="flex items-center space-x-1 sm:space-x-1.5 mt-0.5 flex-wrap">
                <p className="text-[9px] sm:text-[10px] md:text-xs font-medium text-[#FF8C42] hidden sm:inline">Staff Dashboard</p>
                {currentBranch && (
                  <>
      ...
      {showMobileMenu && createPortal(
        <div className="fixed inset-0 z-[9999] overflow-hidden">
          ...
          {/* Menu Content */}
          <div 
            className="absolute inset-0 bg-gradient-to-br from-[#0A0A0A] via-[#1A1A1A] to-[#0F0F0F] overflow-y-auto"
            style={{ paddingTop: 'env(safe-area-inset-top)', paddingBottom: 'env(safe-area-inset-bottom)' }}
          >
            {/* Header */}
            <div className="sticky top-0 z-10 bg-[#0A0A0A]/95 backdrop-blur-md border-b border-[#2A2A2A]/50 px-4 py-3 flex items-center justify-between">
              <div className="flex items-center space-x-3">
                <div className="w-10 h-10 bg-[#0A0A0A] rounded-lg flex items-center justify-center shadow-lg ring-1 ring-[#FF8C42]/20 p-1.5 border border-[#1A1A1A]/50">
                  <img
                    src="/img/tipuno_x_logo_white.avif"
                    alt="TipunoX Logo"
                    className="w-full h-full object-contain"
